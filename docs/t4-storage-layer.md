...
| Retention policy | Prune rows not seen during latest refresh or falling outside the week | `last_seen_at` tracks refresh cycles so missing events disappear after a refresh |
...
2. Executes an upsert keyed on `event_key` using `ON CONFLICT(event_key) DO UPDATE` that replaces mutable fields (title/category/description/organizer/performers/tags/etc.) but rejects changes to identity material (`identity_kind`, `identity_inputs`) for an existing key (raises and leaves the row unchanged).
...
| `last_seen_at` | `TEXT` | NOT NULL | UTC timestamp for the most recent refresh (`YYYY-MM-DDTHH:MM:SS.ssssssZ` preferred to ensure uniqueness), matching `refresh_timestamp` format. |
...
- `refresh_timestamp` is the refresh start time supplied by the orchestrator; it must be timezone-aware UTC (`YYYY-MM-DDTHH:MM:SS.ssssssZ` preferred to avoid collisions). `upsert_events` and `prune_stale_events` must receive the same value, even if the refresh spans midnight. Implementations may accept an optional unique `refresh_id` to guarantee ordering if two refreshes start within the same second.
...
- `prune_stale_events` deletes rows whose `last_seen_at < refresh_timestamp` (meaning they were missing in the current refresh) *or* whose `starts_at` is outside `window` after converting `WeekWindow` bounds to UTC strings and comparing in SQL; start is inclusive, end is exclusive, so equality on the upper bound prunes.
...
3. Call `prune_stale_events(window, refresh_started_at)` once all candidates have been processed and the refresh has succeeded (skip on failed/partial refresh to avoid data loss).
...
- `test_storage_repository_prune_by_window`: An event outside the `WeekWindow` is pruned even if `last_seen_at` matches; include a DST/weekly-boundary case by converting stored UTC to `America/New_York`.
