...
- `get_events_for_window` reads rows whose `starts_at` falls inside `window` using `utc_bounds_for_window(window)` (start inclusive/end exclusive, canonical `YYYY-MM-DDTHH:MM:SSZ`) to build UTC string bounds and returns results sorted by `starts_at` ascending, then `venue_name`, then `event_key`.
- `prune_stale_events` deletes rows whose `last_seen_at < refresh_timestamp` (meaning they were missing in the current refresh) *or* whose `starts_at` is outside `window` using `utc_bounds_for_window(window)` (start inclusive, end exclusive, canonical `YYYY-MM-DDTHH:MM:SSZ`) and comparing in SQL. If any upsert raises (e.g., identity mismatch), the refresh is considered failed/partial and `prune_stale_events` must be skipped.
